...
# NOTE: _select_serial_port_at_startup() now runs inside the FastAPI lifespan
# startup hook so importing this module (tests, reloaders) does not block on stdin.
...
# on Linux (Raspberry Pi). The read loop treats that as a lost link and
# reconnects — which is the spurious "auto reconnect on every uplink" bug.
# So other threads hand outgoing frames to _tx_queue and the reader drains it.
