if (!window.__DGS_BOOTED__) {
  ...
  (function () {
    ...
    function speak(text) {
      ...
      // This avoids speech getting cut off when state oscillates rapidly.
      const _now = Date.now();
      if (window.speechSynthesis.speaking && _now - _lastSpeechMs < 1500) return;
    ...
    function stopMissionTimer(reason) {
      ...
      info(msg);
      // Persisted in ground.jsonl via the backend.
    ...
    function initCesium3D() {
      if (!el.mapEl) return;
      // We don't use Cesium Ion (using ArcGIS imagery + ellipsoid terrain only).
      // Setting to undefined silences the Ion token warning on newer Cesium versions.
    ...
    function onTelemetry(t) {
      ...
      // If we sent "CX,ON" and payload now echoes "CX,ON" (or contains it), mark VERIFIED.
      if (t.cmd_echo) {
    ...
    function connect() {
      // Don't open a second socket if one is already live OR still connecting.
      // Guarding only on OPEN let a second socket spawn while the first was still
      // in the CONNECTING handshake (e.g. a tab-focus/visibilitychange/wake fired
      // mid-connect). st.ws would then point at the new socket while the orphaned
      // one kept its onmessage handler alive — so every telemetry packet was
      // delivered twice and rendered twice in the feed (and double-fed to charts).
      if (st.ws && (st.ws.readyState === WebSocket.OPEN ||
                    st.ws.readyState === WebSocket.CONNECTING)) return;

      // Belt-and-suspenders: detach handlers from any stale socket reference so a
      // lingering half-open connection can never deliver alongside the new one.
      if (st.ws) {
        try {
