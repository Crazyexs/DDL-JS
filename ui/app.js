if (!window.__DGS_BOOTED__) {
  ...
  (function () {
    ...
    // Triggered automatically by the server when the CanSat LANDS (kml_saved WS event),
    // or manually via button (calls /api/kml/save then opens /api/kml).
      ...
      if (t.state && t.state.trim() !== '' && t.state !== st.lastSpokenState) {
        if (st.lastSpokenState !== null && !st.isReplaying) {
          if (t.state === 'LANDED') {
            ...
            // KML auto-save is handled server-side (kml_saved WebSocket event)
          } else {
